...
class NICCControllerSpec extends AnyWordSpec with Matchers {
  ...
  "GET /nationalInsuranceNumber-info" should {
    "return 200" in {
      val requestNino = Nino("AA123456B")
      val requestStartYear = "2017"
      val requestEndYear = "2019"
      val requestDoB = "1998-04-23"
      val requestDoBJson = "{\"dateOfBirth\": \"1998-04-23\"}"
      val requestObject = NICCRequest(requestNino, requestStartYear, requestEndYear, requestDoB)
      val fakeRequest = FakeRequest[String]("POST", "/nicc-json-service/nicc/v1/api/national-insurance/AA123456B/from/2017/to/2019", FakeHeaders(Seq((CONTENT_TYPE, "application/json"))), body = requestDoBJson)
