...
object NiContributionsAndCreditsSuccess {
  ...
  final case class ContributionCategoryLetter private (value: String)

  object ContributionCategoryLetter {
    private val pattern = "^[A-Za-z' -]{2,99}$".r

    private def from(value: String): ValidatedNel[String, ContributionCategoryLetter] =
      Validated.condNel(pattern.matches(value), ContributionCategoryLetter(value), "invalid surname")

    implicit val contributionCategoryLetterReads: Reads[ContributionCategoryLetter] = {
      case JsString(value) =>
    ...
    implicit val contributionCategoryLetterWrites: Writes[ContributionCategoryLetter] =
      Json.writes[ContributionCategoryLetter]

  }

  final case class EmployerName private (value: String)

  object EmployerName {
    private val pattern = "^[A-Za-z' -]{2,99}$".r

    private def from(value: String): ValidatedNel[String, EmployerName] =
      Validated.condNel(pattern.matches(value), EmployerName(value), "invalid surname")

    implicit val employerNameReads: Reads[EmployerName] = {
      case JsString(value) =>
        EmployerName
          .from(value)
          .leftMap(errors => JsError(__ -> JsonValidationError(errors.toList.mkString(","))))
