...
class MarriageDetailsRequestHelper {
  ...
  def buildRequestPath(host: String, req: GYSPEligibilityCheckDataRequest): String = {
    def searchStartYear: Option[String] = req.marriageDetails.searchStartYear.map(sY => s"searchStartYear=$sY&")

    def searchEndYear: Option[String] = req.marriageDetails.searchEndYear.map(eY => s"searchEndYear=$eY&")

    def latest: Option[String] = req.marriageDetails.latest.map(l => s"latest=$l&")

    def sequence: Option[String] = req.marriageDetails.sequence.map(s => s"sequence=$s")

    val options = searchStartYear.combine(searchEndYear).combine(latest).combine(sequence.map(result => s"?$result"))
    s"$host/ni/individual/${req.nationalInsuranceNumber}/marriage-cp$options/"
  }
