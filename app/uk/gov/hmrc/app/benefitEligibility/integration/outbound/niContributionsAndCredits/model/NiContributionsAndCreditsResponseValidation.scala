...
import NiContributionsAndCreditsSuccess.NiContributionsAndCreditsSuccessResponse
...
object NiContributionsAndCreditsResponseValidation {
  ...
  implicit val niContributionsAndCreditsSuccessResponseValidator
    ...
    (benefitType, response: NiContributionsAndCreditsSuccessResponse) =>
      benefitType match {
        case BenefitType.MA =>
          (if (response.class1ContributionAndCredits.isDefined) {
             response.class1ContributionAndCredits.get.flatMap { el =>
               List(
             ...
             response.class2ContributionAndCredits.get.flatMap { el =>
               List(
                 el.taxYear.map(NumberValidation.validate(_, 1900, 2099)),
                 el.numberOfContributionsAndCredits.map(NumberValidation.validate(_, 0, 53))
               ).flatten
             }
