...
import uk.gov.hmrc.app.benefitEligibility.common.ApiName.ContributionCredit
  ...
  def toApiResult(
    ...
    response match {
      case NiContributionsAndCreditsError.NiContributionsAndCreditsResponse400(failures) =>
        DownstreamErrorReport(ContributionCredit, BadRequest)
      case NiContributionsAndCreditsError.NiContributionsAndCreditsResponse403(reason, code) =>
        DownstreamErrorReport(ContributionCredit, AccessForbidden)
      case NiContributionsAndCreditsError.NiContributionsAndCreditsResponse422(failures) =>
        DownstreamErrorReport(ContributionCredit, UnprocessableEntity)
      case response: NiContributionsAndCreditsSuccessResponse => DownstreamSuccessResponse(ContributionCredit, response)
    }
