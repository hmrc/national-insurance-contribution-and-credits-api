    ...
    (_, response: BenefitCalculationDetailsSuccessResponse) =>
      response.benefitCalculationDetailsList
        .map { bCDLList =>
          bCDLList
            .map { bCDL =>
              bCDL.benefitCalculationDetail
                .map { bCD =>
                  List(
                    bCD.guaranteedMinimumPensionContractedOutDeductionsPre1988
                      .map(g => MoneyValidation.validateUnsigned(g)),
                    bCD.guaranteedMinimumPensionContractedOutDeductionsPost1988
                      .map(g => MoneyValidation.validateUnsigned(g)),
                    bCD.contractedOutDeductionsPre1988
                      .map(c => MoneyValidation.validateUnsigned(c)),
                    bCD.contractedOutDeductionsPost1988
                      .map(c => MoneyValidation.validateUnsigned(c))
