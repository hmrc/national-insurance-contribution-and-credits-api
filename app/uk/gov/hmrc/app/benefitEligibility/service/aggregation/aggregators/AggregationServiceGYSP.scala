...
object AggregationServiceGYSP {
  ...
  val aggregator: ResultAggregator[EligibilityCheckDataResultGYSP] =
    (eligibilityCheckDataSuccessResultGYSP: EligibilityCheckDataResultGYSP) =>
      if (eligibilityCheckDataSuccessResultGYSP.allResults.forall(_.isSuccess)) {
        val marriageDetailsResult = eligibilityCheckDataSuccessResultGYSP.marriageDetailsResult.getSuccess.get
      ...
      } else Left(BenefitEligibilityDataFetchError.from(eligibilityCheckDataSuccessResultGYSP))
