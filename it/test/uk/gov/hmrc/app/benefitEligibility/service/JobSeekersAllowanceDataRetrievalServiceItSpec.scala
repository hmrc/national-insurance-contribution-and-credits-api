...
class JobSeekersAllowanceDataRetrievalServiceItSpec
  ...
  private lazy val service: JobSeekersAllowanceDataRetrievalService = inject[JobSeekersAllowanceDataRetrievalService]
    ...
    ".fetchEligibilityData" - {
      ...
      "when the NiContributionsAndCredits endpoint returns OK (200) with valid response" - {
        "should parse response and map to result successfully" in {
          ...
          val result = service.fetchEligibilityData(jsaEligibilityCheckDataRequest).value.futureValue
