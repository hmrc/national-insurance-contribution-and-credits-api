  ...
  "BenefitEligibilityController" - {
    ".fetchBenefitEligibilityData" - {
        ...
        "should fetch ESA Data correctly" in {
          ...
          server.stubFor(
            post(urlEqualTo(npsCreditsAndContributionsPath))
              ...
              .withHeader("gov-uk-originator-id", EqualToPattern("originatorIdEsa"))
              .willReturn(
                aResponse()
                  .withStatus(OK)
                  .withHeader("Content-Type", "application/json")
      ...
      "JSA" - {
        "should fetch JSA Data correctly" in {
          ...
          server.stubFor(
            post(urlEqualTo(npsCreditsAndContributionsPath))
              .withHeader("CorrelationId", EqualToPattern(correlationId.value.toString))
              .withHeader("gov-uk-originator-id", EqualToPattern("originatorIdJsa"))
              .willReturn(
                aResponse()
                  .withStatus(OK)
                  .withHeader("Content-Type", "application/json")
          ...
          val request: FakeRequest[AnyContent] = FakeRequest("POST", "/benefit-eligibility-info")
            ...
            .withHeaders(
              ...
              "CorrelationID"        -> correlationId.value.toString,
              "Accept"               -> "application/json",
              "gov-uk-originator-id" -> "DWP-CF-JSA-6"
            )
      ...
      "should return 400 if request is missing originator id" in {
        ...
        status(result) shouldBe 400
        contentAsJson(result) shouldBe Json.toJson(
          ErrorResponse(BadRequest, ErrorReason("Missing header 'gov-uk-originator-id'"))
        )
      ...
      "should return 400 if request has invalid originator id" in {
        ...
        val request: FakeRequest[AnyContent] = FakeRequest("POST", "/benefit-eligibility-info")
          .withHeaders(
            ...
            "CorrelationID"        -> correlationId.value.toString,
            "Accept"               -> "application/json",
            "gov-uk-originator-id" -> "ImInvalid"
          )
